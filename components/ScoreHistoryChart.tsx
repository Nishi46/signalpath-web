...
export function ScoreHistoryChart({ clusterId }: ScoreHistoryChartProps) {
  ...
  if (loading) {
    return <div className='h-16 bg-gray-50 dark:bg-white/[0.04] rounded-xl animate-pulse' />
  }
  ...
  return (
    <div>
      <div className='flex items-center justify-between mb-2'>
        <span className='text-xs text-gray-500 dark:text-white/40 font-medium flex items-center gap-1.5'>
          <TrendingUp className='w-3.5 h-3.5 text-gray-400 dark:text-white/25' />
      ...
      <div className='relative bg-gray-50 dark:bg-white/[0.04] rounded-xl p-2 overflow-hidden'>
      ...
      {transitions.length > 1 && (
        <div className='flex flex-wrap gap-3 mt-2'>
          {transitions.map((t, i) => (
            <span key={i} className='flex items-center gap-1 text-[10px] text-gray-500 dark:text-white/35'>
              <span
